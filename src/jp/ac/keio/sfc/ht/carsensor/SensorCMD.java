...
public abstract class SensorCMD extends Object {
	 ...
	 */
	/*
	 * final public static byte EVENT_DATA_ERR = (byte) 0x87; final public
	 * static int EVENT_DATA_ERR_PARA_SIZE = 6; final public static int
	 * EVENT_DATA_ERR_SERO_SIZE = 2; final public static int
	 * EVENT_DATA_ERR_DATA_INDEX_SIZE = 3; final public static int
	 * EVENT_DATA_ERR_ERR_SIZE = 1;
	 */
	...
	public static Map<Byte, String> EVENT_DATA_ERR_ERRS = ImmutableMap.<Byte, String>builder()
			...
			.put(new Byte((byte) 0x40), "[Sensor Error: GSP sensor.]").build();
	 ...
	 */
	final public static byte[] START_SENSOR_WITH_FAN = { PROTOCOL_HEADER, CMD_SENSOR, CMD_SENSOR_START, CMD_WITH_FAN,
			(byte) 0x76 };
	/**
	 * Stop Sensor with Fan Working
	 */
	final public static byte[] STOP_SENSOR_WITH_FAN = { PROTOCOL_HEADER, CMD_SENSOR, CMD_SENSOR_STOP, CMD_WITH_FAN,
			(byte) 0x89 };
	/**
	 * Start Sensor without Fan Working
	 */
	final public static byte[] START_SENSOR_WITHOUT_FAN = { PROTOCOL_HEADER, CMD_SENSOR, CMD_SENSOR_START,
			CMD_WITHOUT_FAN, (byte) 0x77 };
	/**
	 * Stop Sensor without Fan Working
	 */
	final public static byte[] STOP_SENSOR_WITHOUT_FAN = { PROTOCOL_HEADER, CMD_SENSOR, CMD_SENSOR_STOP,
			CMD_WITHOUT_FAN, (byte) 0x88 };

	final public static byte[] GET_GPS_STATUS = { PROTOCOL_HEADER, CMD_GET_GPS, (byte) 0x00,
			(byte) PROTOCOL_HEADER ^ CMD_GET_GPS };
	final public static byte[] GET_VS_VALUE = { PROTOCOL_HEADER, CMD_GET_VS, (byte) 0x00,
			(byte) PROTOCOL_HEADER ^ CMD_GET_VS };
	...
	/**
	 * check the BBC value of a byte array with length len. The final byte
	 * (i.e., raw[len-1]) stores the bitwise xor of the proceeding len-1 bytes.
	 * 
	 * @param raw
	 * @param len
	 * @return true if bitwise xor value of the len-1 bytes equals the last
	...
	public static String bytesToDecimalString(byte[] bytes, int from, int to) {
		byte[] buffer = Arrays.copyOfRange(bytes, from, to);
	...
	public static String nullEndianBytesToUTF8String(byte[] bytes, int from, int to) {
		...
		byte[] buffer = Arrays.copyOfRange(bytes, from, end - 1);
		String msg = "";
		try {
			msg += new String(buffer, "UTF-8");
		} catch (UnsupportedEncodingException e) {
