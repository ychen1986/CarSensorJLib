...
public class SensorEventPublisher implements SensorEventListener, Runnable {
	...
	public SensorEventPublisher(String _soxServer, String _soxUser, String _soxPasswd,
			String _soxDevice, String _serialPort, boolean debug) throws TooManyListenersException {
	...
	public void handleSensorEvent(SensorEvent ev) {
		...
		case Sensor.EVENT_DATA_C:value.setId("GPS Data Event");
		...
		case Sensor.EVENT_DATA_B:value.setId("Data Event");
			cmd = ev.getCmd();
		
			
			
				if(dataBuffer == null){
					//System.out.println("bufferspace: " + cmd.length * dataBuffSize);
					dataBuffer = new byte[cmd.length * dataBuffSize];
				}
				//System.out.println("Copy from" + cmd.length * buffCount + "to" + (cmd.length * dataBuffSize+cmd.length));
			...
			break;
		default: value.setId("Response Event");
			cmd = ev.getCmd();		
	...
	protected static void parseOptions(String[] args) {
		if (args.length == 0) {
			System.err
					.println("Usage: java -jar publisher.jar -s <soxServer> -u <soxUser> -p <soxPasswd> -d <soxDevice> -sp <serialPort> -debug <true/flase>");
			System.exit(1);
		}
