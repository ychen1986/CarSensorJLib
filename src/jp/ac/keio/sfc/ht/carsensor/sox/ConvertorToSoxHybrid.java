...
public class ConvertorToSoxHybrid extends ConvertorToSox {
	...
	public ConvertorToSoxHybrid(Socket _socket,
			ObjectInputStream _inFromClient, SoxDevice _high, SoxDevice _low, boolean _debug) {
				...
				if(lowSpeedDvice != null){
					...
					switch (se.getEventType()) {
						case EVENT_DATA_C:
							...
							if(hasLongitude && hasSpeed){// A gps Event with long/lat will incur a publishment.
								break;
							}else{
								continue;
							}
						...
						case EVENT_DATA_A:// Other data are stored into the lastVaules list
